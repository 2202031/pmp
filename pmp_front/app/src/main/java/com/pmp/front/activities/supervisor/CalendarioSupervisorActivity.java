...
public class CalendarioSupervisorActivity extends Activity {
    ...
    // Datos traídos del servidor, en memoria (ya NO se guarda nada en SharedPreferences).
    ...
    // Trae las asignaciones más recientes del servidor (MySQL) y luego
    // vuelve a pintar el calendario usando la lógica local ya existente,
    ...
    private void guardarOActualizarActividad(
        ...
        final String titulo = partes[1];
        final String fecha = partes[2];
        final String hora = partes[3];
        final String descripcion = partes[4];
        final String prioridad = partes[5];
        final String folio = partes[6];
        final String tecnico = partes[7];
        final String estado = partes[8];
        final String fechaRegistro = partes[9];

        executorService.execute(() -> {
            com.pmp.front.ServidorSyncHelper.guardarActividad(
                    idActividad, titulo, fecha, hora, descripcion,
                    prioridad, folio, tecnico, estado, fechaRegistro
            );
            mainHandler.post(this::sincronizarYRecargar);
        });
    }
    ...
    private void eliminarActividad(
        ...
        executorService.execute(() -> {
            com.pmp.front.ServidorSyncHelper.eliminarActividad(idActividad);
            mainHandler.post(() -> {
                sincronizarYRecargar();
                Toast.makeText(
                        this,
