...
public class MisSupervisionesTecnicoActivity extends Activity {
    ...
    /**
     * Ordena las supervisiones: las activas arriba (más recientes primero) y
     * las finalizadas/validadas hasta el final de la cola.
     */
    private String[] ordenarSupervisiones(String[] registros) {
        ...
        java.util.Collections.sort(lista, (a, b) -> {
            ...
            return Long.compare(fechaComoNumero(pb), fechaComoNumero(pa));
        });
    ...
    private View crearTarjetaSupervision(
        ...
        String textoAccion;
        boolean accionHabilitada = checklistCompletado;

        boolean terminada =
                "Finalizada".equalsIgnoreCase(estado) ||
                        "Validada".equalsIgnoreCase(estado) ||
                        "Validado".equalsIgnoreCase(estadoReporte);

        if (terminada) {
            textoAccion = "Finalizada";
            accionHabilitada = false;
        } else if (!checklistCompletado) {
            textoAccion =
                    "Reporte bloqueado";
        } else if (
        ...
        final boolean finalAccionHabilitada = accionHabilitada;

        TextView btnAccion =
                crearBoton(
                        textoAccion,
                        finalAccionHabilitada
                );
